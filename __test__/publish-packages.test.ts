...
describe("publish-packages", () => {
	...
	it("errors when build output directory is missing for non-source target", async () => {
		vi.mocked(getChangesetStatus).mockResolvedValue({
			releases: [{ name: "@org/pkg-a", newVersion: "1.0.0", type: "patch" }],
			changesets: [],
		});
		vi.mocked(findPackagePath).mockReturnValue("/path/to/pkg-a");
		// Source package.json exists, but built target package.json does not
		vi.mocked(fs.existsSync).mockImplementation((p) => {
			const path = String(p);
			// Source package.json exists
			if (path === "/path/to/pkg-a/package.json") return true;
			// Built package.json does NOT exist (build didn't produce it)
			return false;
		});
		vi.mocked(fs.readFileSync).mockReturnValue(
			JSON.stringify({
				...
				publishConfig: {
					access: "public",
					targets: [{ protocol: "npm", registry: "https://npm.pkg.github.com/", directory: "dist/github" }],
				},
			}),
		);
		vi.mocked(exec.exec).mockResolvedValue(0); // Build succeeds
		...
		expect(result.success).toBe(false);
		expect(result.buildError).toContain("Pre-validation failed");
		// Error should mention "Build output directory not found"
		expect(core.error).toHaveBeenCalledWith(expect.stringContaining("Build output directory not found"));
		// publishToTarget should NOT be called - we fail early
		expect(publishToTarget).not.toHaveBeenCalled();
	});
